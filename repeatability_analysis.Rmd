...
summary(sample_sums(meerkat_final))
...
#sample_data(meerkat_filtered)$HydroYear<-factor(lubridate::year(sample_data(meerkat_filtered)$HydroDate))
...
summary(data.frame(table(sample_data(meerkat_icc_asv)$IndividID))$Freq)
...
#write.csv(icc_asv_df, "icc_asv_df.csv")
...
#write.csv(icc_genus_df, "icc_genus_df.csv")
...
# NMDS
#ord_wunifrac_nmds<-ordinate(meerkat_beta, "NMDS", "wunifrac")
#plot_ordination(meerkat_beta, ord_wunifrac_nmds, type="sample", color="SampleTimeCat")
...
summary(gammodel_WU2)

gam_boot<-rptGam::rptgam(gamObj = gammodel_WU2, nboot = 10, nperm = 0)
...
gam_WU2<-data.frame(rbind(c(icc_year_gam, lower_year_gam, upper_year_gam), c(icc_id_gam, lower_id_gam, upper_id_gam), c(icc_group_gam, lower_group_gam, upper_group_gam)))

gam_WU2$effect<-c("Year", "ID", "Group")
gam_WU2$model<-"W_Unifrac_2"
names(gam_WU2)<-c("ICC", "lower", "upper", "effect", "model")

# add p value

gam_WU2$p_val<-gam_boot$lrt$p_value
gam_WU2$Significant <-gam_WU2$p_val < 0.0511

gam_WU2

#######################

gammodel_UU2<- mgcv::bam(MDS2_UU~
                         s(scale(HoursAfterSunrise), bs = "cr")+
                         s(scale(Seq_depth_nospikein), bs = "cr")+
                         s(scale(log10(AgeAtSamplingYears)), bs = "cr")+
                         Storage+
...
summary(gammodel_UU2)

gam_boot<-rptGam::rptgam(gamObj = gammodel_UU2, nboot = 10, nperm = 0)
...
gam_UU2<-data.frame(rbind(c(icc_year_gam, lower_year_gam, upper_year_gam), c(icc_id_gam, lower_id_gam, upper_id_gam), c(icc_group_gam, lower_group_gam, upper_group_gam)))

gam_UU2$effect<-c("Year", "ID", "Group")
gam_UU2$model<-"U_Unifrac_2"
names(gam_UU2)<-c("ICC", "lower", "upper", "effect", "model")

# add p value

gam_UU2$p_val<-gam_boot$lrt$p_value
gam_UU2$Significant <-gam_UU2$p_val < 0.051
...
summary(gammodel_BC2)

gam_boot<-rptGam::rptgam(gamObj = gammodel_BC2, nboot = 10, nperm = 0)
...
gam_BC2<-data.frame(rbind(c(icc_year_gam, lower_year_gam, upper_year_gam), c(icc_id_gam, lower_id_gam, upper_id_gam), c(icc_group_gam, lower_group_gam, upper_group_gam)))

gam_BC2$effect<-c("Year", "ID", "Group")
gam_BC2$model<-"BrayCurtis_2"
names(gam_BC2)<-c("ICC", "lower", "upper", "effect", "model")

# add p value

gam_BC2$p_val<-gam_boot$lrt$p_value
gam_BC2$Significant <-gam_BC2$p_val < 0.051
...
beta_icc_df<-rbind(gam_WU1,gam_WU2, gam_UU1, gam_UU2, gam_BC1, gam_BC2)

ggplot(beta_icc_df, aes(y = ICC, x = effect, col = model))+
  geom_point(size = 2)+
  geom_errorbar(aes(ymin = lower, ymax = upper, linetype = Significant), width = 0)+
...
div_icc_df$model<-factor(div_icc_df$model, levels = rev(c("Bacterial load", "Observed", "Shannon", "PD", "W_Unifrac_1", "W_Unifrac_2", "U_Unifrac_1","U_Unifrac_2", "BrayCurtis_1", "BrayCurtis_2")))
...
taxtable$Genus<-ifelse(is.na(taxtable$Genus), paste("Unknown", taxtable$Family, sep = " "), as.character(taxtable$Genus))
taxtable$Genus<-factor(taxtable$Genus)
taxtable$New_Taxa<-factor(taxtable$New_Taxa)
...
icc_asv_phylosignal<-subset(icc_asv_df, effect == "ID" & model == "GAM gaussian")
summary(icc_asv_phylosignal$ICC)
head(icc_asv_phylosignal)
dim(icc_asv_phylosignal)
...
p4d_id <- phylobase::phylo4d(tree,icc_asv_phylosignal2)
phyloSignal(p4d_id, method = "all")

barplot.phylo4d(p4d_id, tree.type = "phylo",tree.ladderize = F, bar.lwd = 3, show.tip = T, center = T, scale = F, trait.bg.col = "white", grid.col = "grey", tree.open.crown = 0, tip.adj = 1)

scaled_icc_id<-as.numeric(scale(icc_asv_phylosignal2$ICC))
colors_id<-ifelse(scaled_icc_id <=0, "black", "red")

barplot.phylo4d(p4d_id, tree.type = "phylo",tree.ladderize = T, bar.lwd = 4, show.tip = T, center = T, scale = F,  grid.col = "grey", bar.col = colors_id, tip.col = colors_id, tip.cex = 0.7)

barplot.phylo4d(p4d_id, tree.type = "phylo",tree.ladderize = T, bar.lwd = 4, show.tip = F, center = T, scale = F,  grid.col = "grey", bar.col = colors_id, tip.col = colors_id, tip.cex = 0.7)
...
icc_asv_phylosignal<-subset(icc_asv_df, effect == "Group" & model == "GAM gaussian")
summary(icc_asv_phylosignal$ICC)
head(icc_asv_phylosignal)
...
p4d_group <- phylobase::phylo4d(tree,icc_asv_phylosignal2)
phyloSignal(p4d_group, method = "all")

barplot.phylo4d(p4d_group, tree.type = "phylo",tree.ladderize = T, bar.lwd = 3, show.tip = T, center = T, scale = F, trait.bg.col = "white", grid.col = "grey", tree.open.crown = 0, tip.adj = 1)

scaled_icc_group<-as.numeric(scale(icc_asv_phylosignal2$ICC))
colors_group<-ifelse(scaled_icc_group <=0, "black", "red")

barplot.phylo4d(p4d_group, tree.type = "phylo",tree.ladderize = T, bar.lwd = 4, show.tip = T, center = T, scale = F,  grid.col = "grey", bar.col = colors_group, tip.col = colors_group, tip.cex = 0.7)

dev.off()
barplot.phylo4d(p4d_group, tree.type = "phylo",tree.ladderize = T, bar.lwd = 4, show.tip = F, center = T, scale = F,  grid.col = "grey", bar.col = colors_group, tip.col = colors_group, tip.cex = 0.7, tree.ratio =0)
...
```{r}
icc_asv_phylosignal<-subset(icc_asv_df, effect == "Year" & model == "GAM gaussian")
summary(icc_asv_phylosignal$ICC)
head(icc_asv_phylosignal)
...
p4d_year <- phylobase::phylo4d(tree,icc_asv_phylosignal2)
phyloSignal(p4d_year, method = "all")

barplot.phylo4d(p4d_year, tree.type = "phylo",tree.ladderize = F, bar.lwd = 3, show.tip = T, center = T, scale = F, trait.bg.col = "white", grid.col = "grey", tree.open.crown = 0, tip.adj = 1)

scaled_icc_year<-as.numeric(scale(icc_asv_phylosignal2$ICC))
#summary(scaled_icc)
colors_year<-ifelse(scaled_icc_year <=0, "black", "red")


dev.off()
barplot.phylo4d(p4d_year, tree.type = "phylo",tree.ladderize = T, bar.lwd = 4, show.tip = T, center = T, scale = F,  grid.col = "grey", bar.col = colors_year, tip.col = "black", tip.cex = 1, tree.ratio = 0)
...
summary(data.frame(table( pairwise_metadata$Interval))$Freq)
head(pairwise_metadata)
...
# 17. Beta dissimilarity

```{r}
meerkat_nmds<-microbiome::core(meerkat_diversity_ps, detection = 0, prevalence = 0.05)

dist_unifrac <- phyloseq::distance(meerkat_nmds, "unifrac")
#nmds_unifrac <- vegan::metaMDS(dist_unifrac, k = 3, trymax = 40)

dist_unifrac

dist_unifrac_mat<-as.matrix(dist_unifrac)
dist_unifrac_mat[1:5,1:5]

str(dist_unifrac)


bd_df<-harrietr::melt_dist(dist_unifrac_mat)
dim(bd_df)

names(bd_df)<-c("Sample1", "Sample2", "Dist")

head(bd_df)

metadata<-data.frame(sample_data(meerkat_nmds))
head(metadata)

bd_df$Sample1_ID<-vlookup(bd_df$Sample1, metadata, lookup_column = "feature.id", result_column = "IndividID")

bd_df$Sample2_ID<-vlookup(bd_df$Sample2, metadata, lookup_column = "feature.id", result_column = "IndividID")

bd_df$Sample1_date<-vlookup(bd_df$Sample1, metadata, lookup_column = "feature.id", result_column = "SampleDate")

bd_df$Sample2_date<-vlookup(bd_df$Sample2, metadata, lookup_column = "feature.id", result_column = "SampleDate")

bd_df$Sample1_group<-vlookup(bd_df$Sample1, metadata, lookup_column = "feature.id", result_column = "GroupAtSampling")

bd_df$Sample2_group<-vlookup(bd_df$Sample2, metadata, lookup_column = "feature.id", result_column = "GroupAtSampling")


head(bd_df)

bd_df$Comparison<- ifelse(bd_df$Sample1_ID == bd_df$Sample2_ID, "Within ID", "Between ID - different group")

bd_df$Comparison<- ifelse(bd_df$Comparison == "Between ID - different group" &  bd_df$Sample1_group == bd_df$Sample2_group, "Between ID - same group", bd_df$Comparison)

bd_df$Days <-abs(as.numeric(bd_df$Sample1_date - bd_df$Sample2_date))


bd_df$Distance <- "U_Unifrac"
...
dist_wunifrac <- phyloseq::distance(meerkat_nmds, "wunifrac")
#nmds_wunifrac <- vegan::metaMDS(dist_wunifrac, k = 3, trymax = 40)


dist_wunifrac_mat<-as.matrix(dist_wunifrac)
dist_wunifrac_mat[1:5,1:5]

#str(dist_wunifrac)


bd_df_wu<-harrietr::melt_dist(dist_wunifrac_mat)

names(bd_df_wu)<-c("Sample1", "Sample2", "Dist")
...
bd_df_wu$Sample1_ID<-vlookup(bd_df_wu$Sample1, metadata, lookup_column = "feature.id", result_column = "IndividID")

bd_df_wu$Sample2_ID<-vlookup(bd_df_wu$Sample2, metadata, lookup_column = "feature.id", result_column = "IndividID")

bd_df_wu$Sample1_date<-vlookup(bd_df_wu$Sample1, metadata, lookup_column = "feature.id", result_column = "SampleDate")

bd_df_wu$Sample2_date<-vlookup(bd_df_wu$Sample2, metadata, lookup_column = "feature.id", result_column = "SampleDate")

bd_df_wu$Sample1_group<-vlookup(bd_df_wu$Sample1, metadata, lookup_column = "feature.id", result_column = "GroupAtSampling")

bd_df_wu$Sample2_group<-vlookup(bd_df_wu$Sample2, metadata, lookup_column = "feature.id", result_column = "GroupAtSampling")


head(bd_df_wu)

bd_df_wu$Comparison<- ifelse(bd_df_wu$Sample1_ID == bd_df_wu$Sample2_ID, "Within ID", "Between ID - different group")

bd_df_wu$Comparison<- ifelse(bd_df_wu$Comparison == "Between ID - different group" &  bd_df_wu$Sample1_group == bd_df_wu$Sample2_group, "Between ID - same group", bd_df_wu$Comparison)


head(bd_df_wu)


bd_df_wu$Days <-abs(as.numeric(bd_df_wu$Sample1_date - bd_df_wu$Sample2_date))



bd_df_wu$Distance <- "W_Unifrac"
...
dist_bray <- phyloseq::distance(meerkat_nmds, "bray")
#nmds_bray <- vegan::metaMDS(dist_bray, k = 3, trymax = 40)


dist_bray_mat<-as.matrix(dist_bray)
dist_bray_mat[1:5,1:5]

#str(dist_bray)


bd_df_bray<-harrietr::melt_dist(dist_bray_mat)

names(bd_df_bray)<-c("Sample1", "Sample2", "Dist")
...
bd_df_bray$Sample1_ID<-vlookup(bd_df_bray$Sample1, metadata, lookup_column = "feature.id", result_column = "IndividID")

bd_df_bray$Sample2_ID<-vlookup(bd_df_bray$Sample2, metadata, lookup_column = "feature.id", result_column = "IndividID")

bd_df_bray$Sample1_date<-vlookup(bd_df_bray$Sample1, metadata, lookup_column = "feature.id", result_column = "SampleDate")

bd_df_bray$Sample2_date<-vlookup(bd_df_bray$Sample2, metadata, lookup_column = "feature.id", result_column = "SampleDate")


bd_df_bray$Sample1_group<-vlookup(bd_df_bray$Sample1, metadata, lookup_column = "feature.id", result_column = "GroupAtSampling")

bd_df_bray$Sample2_group<-vlookup(bd_df_bray$Sample2, metadata, lookup_column = "feature.id", result_column = "GroupAtSampling")


head(bd_df_bray)

bd_df_bray$Comparison<- ifelse(bd_df_bray$Sample1_ID == bd_df_bray$Sample2_ID, "Within ID", "Between ID - different group")

bd_df_bray$Comparison<- ifelse(bd_df_bray$Comparison == "Between ID - different group" &  bd_df_bray$Sample1_group == bd_df_bray$Sample2_group, "Between ID - same group", bd_df_bray$Comparison)



head(bd_df_bray)

bd_df_bray$Days <-abs(as.numeric(bd_df_bray$Sample1_date - bd_df_bray$Sample2_date))


bd_df_bray$Distance <- "Bray_Curtis"

###########################

beta_diss_df<-rbind(bd_df, bd_df_wu, bd_df_bray)
head(beta_diss_df)

beta_diss_df$Years<-beta_diss_df$Days/365
...
labeller <- function(variable,value){
  return(distance_names[value])
}

ggplot(beta_diss_df, aes(x = Years, y = Dist, group = Comparison, col = Comparison, fill = Comparison))+
  geom_smooth(size = 1)+
  xlim(c(0,2))+
  xlab("Years between sampling")+
  ylab("Beta dissimilarity")+
  facet_wrap(~Distance, ncol = 3, scales = "free_y", labeller=labeller)+
  geom_vline(xintercept = 1, linetype = "longdash", col = "grey")+
  scale_colour_manual(values = c("pink2", "skyblue2", "darkgrey"), guide = guide_legend(reverse = TRUE))+
  scale_fill_manual(values = c("pink", "skyblue", "grey"), guide = guide_legend(reverse = TRUE))+
  theme_bw(base_size = 12)


ggsave("Figures_V2/Fig4.pdf")
...
bd_df_bray$Comparison<-factor(bd_df_bray$Comparison)

bd_df_bray2<-subset(bd_df_bray, Days < 730)
dim(bd_df_bray2)

bray_gam <- mgcv::bam(Dist~
                         s(Days, bs = "cr", by = Comparison),
                       data=bd_df_bray2,
                       family = gaussian)
summary(bray_gam)

confint_df_bray<-confint(bray_gam, parm = "s(Days)",  type = "confidence", partial_match = TRUE)

ggplot(confint_df_bray, aes(x = Days, y = est, col = Comparison))+geom_line()+
  geom_ribbon(aes(ymin=lower, ymax=upper, fill = Comparison))


## unweighted unifrac

bd_df$Comparison<-factor(bd_df$Comparison)

bd_df2<-subset(bd_df, Days < 730)

uu_gam<- mgcv::bam(Dist~
                         s(Days, bs = "cr", by = Comparison),
                       data=bd_df2,
                       family = gaussian)


confint_df_uu<-confint(uu_gam, parm = "s(Days)",  type = "confidence", partial_match = TRUE)

ggplot(confint_df_uu, aes(x = Days, y = est, col = Comparison))+geom_line()+
  geom_ribbon(aes(ymin=lower, ymax=upper, fill = Comparison))
...
bd_df_wu$Comparison<-factor(bd_df_wu$Comparison)

bd_df_wu2<-subset(bd_df_wu, Days < 730)

wu_gam <- mgcv::bam(Dist~
                         s(Days, bs = "cr", by = Comparison),
                       data=bd_df_wu2,
                       family = gaussian)
summary(wu_gam)

confint_df_wu<-confint(wu_gam, parm = "s(Days)",  type = "confidence", partial_match = TRUE)

ggplot(confint_df_wu, aes(x = Days, y = est, col = Comparison))+geom_line()+
  geom_ribbon(aes(ymin=lower, ymax=upper, fill = Comparison))
